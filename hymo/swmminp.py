...
class SWMMInpFile(BaseReader):
    ...
    def _check_headers(self):
        ...
        cards_in_inp = set(
            [_.split("]")[0][1:].lower() for _ in self.orig_file if _[0] == "["]
        )
    ...
    def conduits(self):
        if self._conduits is None:
            ...
            dtype = {"Name": str, "Inlet_Node": str, "Outlet_Node": str}
    ...
    def orifices(self):
        ...
        if self._orifices is None:
            ...
            dtype = {"Name": str, "From_Node": str, "To_Node": str, "Type": str}
    ...
    def xsections(self):
        if self._xsections is None:
            names = ["Link", "Shape", "Geom1", "Geom2", "Geom3", "Geom4", "Barrels"]
    ...
    def losses(self):
        if self._losses is None:
            names = ["Link", "Inlet", "Outlet", "Average", "Flap_Gate", "SeepageRate"]
    ...
    def curves(self):
        if self._curves is None:
            ...
            df = (
                df_raw.assign(
                    _Y_Value=lambda df: np.where(
                        df.Y_Value.isnull(), df.X_Value, df.Y_Value
                    )
                )
                .assign(
                    _X_Value=lambda df: np.where(
                        df.Y_Value.isnull(), df.Type, df.X_Value
                    )
                )
