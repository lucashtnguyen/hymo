...
class SWMMReportFile(BaseReader):
    ...
    def __init__(self, path):
        ...
        # check units
        self.unit = (
            self.orig_file[self.find_line_num("Flow Units")]
            .split(".")[-1]
            .strip()
            .upper()
        )
        ...
        # check swmm version
        self.version = (
            self.orig_file[self.find_line_num("VERSION")].split(" - ")[1].split(" ")[1]
        )
...
class _ReportHeaders(object):
    ...
    def __init__(self, ftype):
        ...
        if self.ftype not in ["CFS", "LPS", "MGD"]:
    ...
    @property
    def subcatchment_summary(self):
        names = ["Name", "Area", "Width", "%Imperv", "%Slope", "Rain_Gage", "Outlet"]
        dtype = {"Name": str}
        return names, dtype
    ...
    def node_inflow_results(self):
        if self.ftype in ["CFS", "MGD"]:
            names = [
                "Node",
                "Type",
                f"Maximum_Lateral_Inflow_{self.ftype}",
                f"Maximum_Total_Inflow_{self.ftype}",
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                "Lateral_Inflow_Volume_mgals",
            ...
            names = [
                "Node",
                "Type",
                "Maximum_Lateral_Inflow_LPS",
                "Maximum_Total_Inflow_LPS",
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                "Lateral_Inflow_Volume_mltr",
                "Total_Inflow_Volume_mltr",
                "Flow_Balance_Error_Percent",
                "flag",
            ]
        dtype = {"Node": str}
    ...
    def node_flooding_results(self):
        if self.ftype in ["CFS", "MGD"]:
            names = [
                "Node",
                "Hours_Flooded",
                f"Maximum_Rate_{self.ftype}",
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                "Total_Flood_Volume_mgal",
            ...
            names = [
                ...
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                "Total_Flood_Volume_mltr",
                "Maximum_Ponded_Depth_Meters",
            ]
    ...
    def storage_volume_results(self):
        if self.ftype in ["CFS", "MGD"]:
            names = [
                "Storage_Unit",
                "Average_Volume_1000_ft3",
                "Avg_Pcnt_Full",
                "Evap_Pcnt_Loss",
                "Exfil_Pcnt_Loss",
                "Maximum_Volume_1000_ft3",
                "Max_Pcnt_Full",
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                f"Maximum_Outflow_{self.ftype}",
            ]
        elif self.ftype == "LPS":
            names = [
                "Storage_Unit",
                "Average_Volume_1000_m3",
                "Avg_Pcnt_Full",
                "Evap_Pcnt_Loss",
                "Exfil_Pcnt_Loss",
                "Maximum_Volume_1000_m3",
                "Max_Pcnt_Full",
                "Time_of_Max_Occurrence_days",
                "Time_of_Max_Occurrence_hours",
                "Maximum_Outflow_LPS",
    ...
    def flow_classification_results(self):
        names = [
            ...
            "Fraction_of_Time_Dry",
            "Fraction_of_Time_Up_Dry",
            "Fraction_of_Time_Down_Dry",
            "Fraction_of_Time_Sub_Crit",
            "Fraction_of_Time_Sup_Crit",
            "Fraction_of_Time_Up_Crit",
            "Fraction_of_Time_Down_Crit",
            "Fraction_of_Time_Norm_Ltd",
